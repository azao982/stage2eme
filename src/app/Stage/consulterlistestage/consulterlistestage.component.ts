...
@Component({
  selector: 'app-consulterlistestage',
  templateUrl: './consulterlistestage.component.html',
  styleUrls: ['./consulterlistestage.component.css']
})
export class ConsulterlistestageComponent implements OnInit {
  ...
  searchStage(): void {
    ...
    this.stageService.searchStage(this.searchKeyword).subscribe(
      ...
      (error) => {
        console.error('Erreur lors de la recherche du stage :', error);
        this.showErrorMessage("Erreur lors de la recherche du stage");
      }
    );
  }
